...
public class PmsUserAction {
	...
	@Autowired
	private UserService pmsUserFacade;
	...
	public String listPmsUser() {
		try {
			...
			User pmsUser = getLoginedUser();// 获取当前登录用户对象
	...
	public String viewPmsUserUI() {
		try {
			...
			User pmsUser = pmsUserFacade.getById(userId);
			if (pmsUser == null) {
	...
	public String addPmsUser() {
		try {
			...
			// 校验用户登录名是否已存在
			User userNoCheck = pmsUserFacade.findUserByUserNo(userNo);
			if (userNoCheck != null) {
	 ...
	 * @return
	 */
	private String validatePmsUser(User user) {
	...
	public String editPmsUserUI() {
		try {
			Long id = getLong("id");
			User pmsUser = pmsUserFacade.getById(id);
			if (pmsUser == null) {
	...
	public String changeUserStatus() {
		try {
			Long userId = getLong("id");
			User user = pmsUserFacade.getById(userId);
			if (user == null) {
	...
	public String resetUserPwdUI() {
		User user = pmsUserFacade.getById(getLong("id"));
		if (user == null) {
	...
	public String userViewOwnInfo() {
		try {
			...
			User pmsUser = this.getLoginedUser();
			if (pmsUser == null) {
				return operateError("无法从会话中获取用户信息");
			}

			User user = pmsUserFacade.getById(pmsUser.getId());
			if (user == null) {
