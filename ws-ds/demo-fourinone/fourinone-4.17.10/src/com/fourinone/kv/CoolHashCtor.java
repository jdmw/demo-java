...
public class CoolHashCtor extends DumpCtor implements CoolHashClient{
	public CoolHashCtor(String parkhost, int parkport){
		super(parkhost,parkport);
	}
	
	public CoolHashResult find(String keywild, Filter filter, boolean point, String... pointSubKey){
		if(CoolHashBase.chex.checkWild(keywild)){
	...
	byte[] dump(String k, byte[] v, byte c, boolean p, String... psk){
		...
		DumpAdapter da = CoolHashBase.dumpAdapter.getKeyMeta(k);
