...
import com.fourinone.pattern.warehouse.*;
...
//	}
	...
	@Delegate(interfaceName=ParkLocal.class,methodName="create",policy= DelegatePolicy.Implements)
	...
	@Delegate(interfaceName=ParkLocal.class,methodName="update",policy=DelegatePolicy.Implements)
	...
	@Delegate(interfaceName=ParkLocal.class,methodName="get",policy=DelegatePolicy.Implements)
	...
	@Delegate(interfaceName=ParkLocal.class,methodName="getLastest",policy=DelegatePolicy.Implements)
	...
	@Delegate(interfaceName=ParkLocal.class,methodName="delete",policy=DelegatePolicy.Implements)
	...
	@Delegate(interfaceName=ParkLocal.class,methodName="setDeletable",policy=DelegatePolicy.Implements)
	...
	@Delegate(interfaceName=ParkLocal.class,methodName="addLastestListener",policy=DelegatePolicy.Implements)
	...
	public ObjectBean OvToBean(ObjValue ov, String domain, String node){
		if(ov!=null&&!ov.isEmpty())
		{
			...
			obp.vid = (Long)ov.getObj(ParkMeta.getVersion(domainnodekey));
	...
	public List<ObjectBean> OvToBeanList(ObjValue ov, String domain){
		if(ov!=null&&!ov.isEmpty())
		{
			...
			objlist.vid = (Long)ov.getObj(ParkMeta.getVersion(domain));
			ObjValue nodeversion = ov.getWidely(ParkMeta.getVersion(domain+"..*"));
			ArrayList<String> nvnames = nodeversion.getObjNames();
			for(String nvname:nvnames){
				ObjectBeanProxy obp = new ObjectBeanProxy();
				obp.vid = (Long)nodeversion.getObj(nvname);
				obp.name = nvname.substring(0,nvname.indexOf(ParkMeta.getMeta()));
