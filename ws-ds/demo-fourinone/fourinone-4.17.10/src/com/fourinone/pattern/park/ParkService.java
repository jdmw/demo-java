...
import com.fourinone.pattern.warehouse.ObjectBytes;
...
public class ParkService extends MementoService implements Park
{
	...
	private SerializeWriter objectBytes;
	...
	private Long updateDomainVersion(String domain) throws RemoteException{
		Long domainversion = (Long)parkinfo.getObj(ParkMeta.getVersion(domain));
		
		ObjValue nodeversions = parkinfo.getWidely(ParkMeta.getVersion(domain+"\\..+"));
			...
			try{
				...
				if(!parkinfo.containsKey(domainnodekey)){
					if(!parkinfo.containsKey(domain)){
						...
						parkinfo.setObj(ParkMeta.getVersion(domain), 0l);
						parkinfo.setString(ParkMeta.getCreateBy(domain), sessionid);
						parkinfo.setString(ParkMeta.getCreateIP(domain), getClientHost());
						parkinfo.setObj(ParkMeta.getCreateTime(domain), System.currentTimeMillis());
					}
					parkinfo.setObj(domainnodekey, obj);
					parkinfo.setObj(ParkMeta.getVersion(domainnodekey), getObjectVersion(obj));
					parkinfo.setObj(ParkMeta.getVersion(domain), updateDomainVersion());//updateDomainVersion(domain)
					parkinfo.setObj(ParkMeta.getCreateBy(domainnodekey), sessionid);
					parkinfo.setObj(ParkMeta.getAuth(domainnodekey), auth);
					parkinfo.setObj(ParkMeta.getCreateIP(domainnodekey), getClientHost());
					parkinfo.setObj(ParkMeta.getCreateTime(domainnodekey), System.currentTimeMillis());
					Long nodenum = (Long)parkinfo.getObj(domain);
					parkinfo.setObj(domain, nodenum+1);
					
					if(heartbeat)
					{
						parkinfo.setObj(ParkMeta.getProp(domainnodekey), ParkMeta.getHeartBeat());
					}
			...
			try{
				...
				if(checkAuth(domainnodekey, sessionid, AuthPolicy.OP_READ_WRITE)){
					if(parkinfo.containsKey(domainnodekey)){
						...
						if(theversion!=(Long)parkinfo.getObj(ParkMeta.getVersion(domainnodekey))){
							parkinfo.setObj(ParkMeta.getVersion(domainnodekey), theversion);
							parkinfo.setObj(ParkMeta.getVersion(domain), updateDomainVersion());//updateDomainVersion(domain)
						}
						parkinfo.setString(ParkMeta.getUpdateBy(domainnodekey), sessionid);
						parkinfo.setString(ParkMeta.getUpdateIP(domainnodekey), getClientHost());
						parkinfo.setObj(ParkMeta.getUpdateTime(domainnodekey), System.currentTimeMillis());
			...
			try{
				if(parkinfo.containsKey(domain)){
					if(checkAuth(domain, sessionid, AuthPolicy.OP_READ_WRITE)){
						parkinfo.setObj(ParkMeta.getAuth(domain), auth);
						updateflag = true;
	...
	public ObjValue delete(String domain, String node)
	{
		ObjValue objrm = null;
		Lock wlk = rwlk.writeLock();
		wlk.lock();
	...
	public boolean checkAuth(String domainnodekey, String sessionid, AuthPolicy targetauth)
	{
		...
		String creator = parkinfo.getString(ParkMeta.getCreateBy(domainnodekey));
		if(creator!=null){
			if(creator.equals(sessionid))
				authflag = true;
			else{
				Object domainnodeAuth = parkinfo.getObj(ParkMeta.getAuth(domainnodekey));
	...
	public ObjValue getLastest(String domain, String node, String sessionid, long version) throws RemoteException,ClosetoOverException
	{
		...
		Lock rlk = rwlk.readLock();
		rlk.lock();
		Long nodeversion = (Long)parkinfo.getObj(ParkMeta.getVersion(parkinfo.getDomainnodekey(domain, node)));
	...
	public ObjValue getTheParkinfo()
	{
		ObjValue ov = null;
		Lock rlk = rwlk.readLock();
		rlk.lock();
