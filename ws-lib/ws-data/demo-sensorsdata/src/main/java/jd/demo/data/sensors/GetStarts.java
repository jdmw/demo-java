...
/**
 * Created by huangxia on 2019/6/20.
 */
public class GetStarts {

    static final String SA_SERVER_URL = "http://localhost/sa" ;
    // 使用 Debug 模式，并且导入 Debug 模式下所发送的数据
    static final boolean SA_WRITE_DATA = true;

    static String DEVICE_ID = UUID.randomUUID().toString();
    static String USER_ID = "U" + String.format("%3d",System.currentTimeMillis()%1000);
    ...
    public static void main(String[] args) throws InvalidArgumentException, IOException, ParseException {
        //SensorsAnalytics sa = new SensorsAnalytics(new SensorsAnalytics.DebugConsumer(SA_SERVER_URL,SA_WRITE_DATA));
        Writer writer = new PrintWriter(System.out);
        SensorsAnalytics sa = new SensorsAnalytics(new SensorsAnalytics.ConsoleConsumer(writer));
        //SensorsAnalytics sa = new SensorsAnalytics(new SensorsAnalytics.ConcurrentLoggingConsumer(new File("sa.data").getAbsolutePath(),"sa"));

        Map<String, Object> properties = new HashMap<String, Object>();
        // 服务器应用版本
        properties.put("ServerVersion", "1.0");
        // 服务器机房地址
        properties.put("Location", "ShangHai");
        // 设置事件公共属性
        sa.registerSuperProperties(properties);

        // 设置用户渠道属性，如果属性已存在，则忽略而不覆盖已有数据
        sa.profileSetOnce(DEVICE_ID, false, "AdSource", "App Store");
        sa.profileSetOnce(DEVICE_ID, false, "ActiveTime", new Date());

        // 记录用户浏览商品事件
        sa.track(DEVICE_ID,false,"ViewProduct",viewProductProperties("Food","Bread001"));
        sa.track(DEVICE_ID,false,"ViewProduct",viewProductProperties("Food","Bread002"));
        sa.track(DEVICE_ID,false,"ViewProduct",viewProductProperties("Food","Bread003"));
        sa.track(DEVICE_ID,false,"ViewProduct",viewProductProperties("Food","Bread004"));
        sa.track(DEVICE_ID,false,"ViewProduct",viewProductProperties("Food","Bread005"));

        writer.flush();

        // 用户登录
        sa.trackSignUp(USER_ID,DEVICE_ID);
        sa.profileSet(USER_ID, true, "Sex", "Male");
        sa.profileSet(USER_ID, true, "Level", "VIP");

        sa.track(USER_ID,true,"ViewProduct",viewProductProperties("Food","Bread001"));
        sa.track(USER_ID,true,"ViewProduct",viewProductProperties("Food","Bread006"));



        // 记录用户浏览商品事件
        sa.track(USER_ID,true,"PaidOrder",paidOrderProperties("O12",Arrays.asList("Bread001","Bread006"),13));
    ...
    private static Map<String,Object> viewProductProperties(String catalog, String productId){
        return viewProductProperties(catalog,productId,false);
    }
     ...
     * @param addToFav 是否加入收藏夹
     * @return
     */
    private static Map<String,Object> viewProductProperties(String catalog, String productId, boolean addToFav){
        Map<String, Object> properties = new HashMap<String, Object>();
        ...
        properties.put("ProductCatalog", catalog);
     ...
     * @return
     */
    private static Map<String,Object> paidOrderProperties(String orderId, List<String> productIds, double amount){
        Map<String, Object> properties = new HashMap<String, Object>();
        properties.put("$ip", IP);
        properties.put("OrderId", orderId);
        properties.put("ProductIdList", productIds);
        properties.put("OrderPaid", amount);
        return properties;
    }


    private static void demo(SensorsAnalytics sa) throws InvalidArgumentException, ParseException {
        /*
